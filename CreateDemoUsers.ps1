...
#
# User properties
$ou = "OU=TestUsers,DC=adtest,DC=Local"      # Which OU to create the user in
$initialPassword = "Password1"               # Initial password set for the user
$orgShortName = "AC"                         # This is used to build a user's sAMAccountName
$dnsDomain = "adtest.local"                  # Domain is used for e-mail address and UPN
...
[System.Collections.ArrayList]$phoneCountryCodes = @{"NL" = "+31"; "GB" = "+44"; "DE" = "+49"; "US" = "+1" }         # Country codes for the countries used in the address file

# Other parameters
$userCount = 2000                           # How many users to create
...
if ($i -lt $userCount) {
   foreach ($firstname in $firstNames) {
      foreach ($lastname in $lastnames) {
         ...
         $officePhone = $matchcc.Value + " " + $postalAreaCodes[$postalCode] + "-" + (Get-Random -Minimum 1000 -Maximum 9999)
